// Cloudflare Worker 入口：API（fetch handler）+ 入库 Workflow。前端静态资源由 assets 绑定托管
// （wrangler.jsonc：/api/* 先进 Worker，其余走 assets，SPA 未命中回退 index.html）。
// 串起混合检索（#4）+ 多轮记忆（#5）+ 联网兜底（#6）+ 入库上云（R2 + Workflow）。
// 由 src/web.ts(node:http) 端口化而来：路由逻辑一致，仅换 Request/Response/Workflow 形态。
...
// 存进 R2 → 启动入库 Workflow（OCR→切块→embed→upsert，自带持久/重试）→ 返回 instanceId。
// 前端轮询 /api/ingest/status 读 Workflow 写在 R2 的进度。
...
// 入库进度：先读 Workflow 写在 R2 的进度 JSON；缺失则回退查 Workflow 实例状态。
async function handleIngestStatus(env: Env, url: URL): Promise<Response> {
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    // 非 /api 一般已被 assets 直接处理；兜底交回 assets（SPA 回退 index.html）。
