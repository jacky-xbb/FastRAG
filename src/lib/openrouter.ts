 ...
 *  优先连远程 Turso（TURSO_DATABASE_URL + TURSO_AUTH_TOKEN）；否则用 VECTOR_DB_URL
 *  （fly.io 设 file:/data/vector.db，也可指向切法 A/B 对比库）；都不设则回退本地 `file:./vector.db`。 */
export const VECTOR_DB_URL =
  process.env.TURSO_DATABASE_URL ?? process.env.VECTOR_DB_URL ?? 'file:./vector.db'
export const VECTOR_DB_AUTH_TOKEN = process.env.TURSO_AUTH_TOKEN
