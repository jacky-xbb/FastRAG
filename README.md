...
> 📊 **召回率评测**：线上库 Recall@6 — 指标 **100%**（36/36）、正文裸召回 **87.5%**（28/32，检索底线，经 Agent 改写后更高），端到端来源正确 **94.4%**。完整报告见 [test/recall-2026-06-17.md](test/recall-2026-06-17.md)。
...
召回率评测结果（线上库 Recall@6，2026-06-17）：

| 口径 | 结果 |
| --- | --- |
| 指标·裸召回 | **36/36 = 100%**（带号 27/27、不带号 9/9） |
| 指标·标准号过滤（产线路径） | **36/36 = 100%** |
| 正文·召回（`--prose`） | **28/32 = 87.5%**（带号 91.7%、不带号 85%） |
| 端到端·来源正确（`--llm`，P2） | 召回 36/36=100%，来源正确 **34/36 = 94.4%** |

逐题明细见 [test/recall-2026-06-17.md](test/recall-2026-06-17.md)。
