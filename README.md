...
对话界面（Issue #7）：`npm run web` 起一个本地 web UI（Node 内置 http，无前端框架），
复用 `standardsAgent` 串起混合检索 + 多轮记忆 + 联网兜底。浏览器开 `http://localhost:4111`（可 `PORT=` 改端口）。
同一页面用一个 threadId 贯穿多轮，答案按 Agent 原文展示来源（文件名+页码 / 联网链接）；
命中废止标准时（`src/lib/answer.ts` 的 `hasDeprecatedNotice` 识别）气泡顶部加「⚠ 含已作废标准」红标。
